let counter = 0;
let colorSchemes = ['white', 'black', 'red']; // Add more color schemes as needed
...
document.getElementById('employeeStatementButton').addEventListener('click', function() {
...
function changeColorScheme() {
   ...
   let randomColorIndex = Math.floor(Math.random() * colorSchemes.length);
   let randomColor = colorSchemes[randomColorIndex];
   body.style.backgroundColor = randomColor;
   navBar.style.backgroundColor = randomColor === 'white' ? 'black' : 'white';
   navBar.style.color = randomColor === 'white' ? 'black' : 'white';
}
function showEmployeeStatement() {
   let statements = [
       "Statement 1: Our team loves working at Air Canada!",
       "Statement 2: Air Canada is more than just a workplace, it's a family!",
       "Statement 3: Proud to be part of the Air Canada family!",
       "Statement 4: Air Canada - where passion meets excellence!"
   ];
   let statementIndex = Math.floor(Math.random() * statements.length);
   let statement = statements[statementIndex];
   let statementDiv = document.getElementById('employeeStatement');
